...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (token) {
      // إذا كان التوكن موجودًا نرسل طلبًا للتحقق من صحته
      axios
        .get("http://localhost:8000/api/check-auth", {
      ...
      setLoading(false);
      // إذا لم يكن هناك توكن، ننهي حالة الانتظار فوراً
    }
  }, []); // ملاحظة: عند الحاجة لإعادة تحميل في حال تغير التوكن، يمكن استخدام [token] كـ dependency
