...
function UsedCarsPage() {
  ...
  useEffect(() => {
    ...
    // جلب السيارات المستعملة فقط التي تم الموافقة عليها (approved=1)
    axios
      .get('http://localhost:8000/api/admin/cars/used', config)
      .then((response) => {
        setCars(response.data || []);
        setLoading(false);
      })
      .catch((error) => {
        setError('فشل في جلب البيانات الخاصة بالسيارات المستعملة');
        setLoading(false);
      });
  }, []);
  ...
  return (
    <div className="used-cars-page">
      <h2>إدارة السيارات المستعملة</h2>
              ...
              <React.Fragment key={car.id}>
                <tr>
                  ...
                  <td>
                    <Button
                      variant="info"
                      onClick={() => handleShowDetails(car.id)} // عند الضغط يتم عرض التفاصيل
                      className="me-2"
                    >
                      تفاصيل الإعلان
                    </Button>
